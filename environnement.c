...
void enlever_env(object* list_env) {
    object list_env_buff = cdr(*list_env);
    *list_env = list_env_buff;
    return;
}

void ajouter_env(object env, object* list_env) {
    object list_env_buff=make_pair(env,*list_env);
    *list_env = list_env_buff;
    return;
}

void ajouter_variable(object* list_env,object symbole,object valeur) {
    object binding=make_pair(symbole,valeur);
...
void ajouter_binding_list_env(object binding, object* list_env)
{
    object env_courant=car(*list_env);
    object env_buff = make_pair(binding, env_courant);
    enlever_env(list_env);
    ajouter_env(env_buff,list_env);
    return;
}
